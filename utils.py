...
def analyze_ObjectAttributes(func_name, state, ObjectAttributes):
    ...
    logger.debug(f"Name: {ObjectName}, Buffer: {Buffer}, Attributes: {Attributes}")
    object_name_struct = state.mem[ObjectAttributes].OBJECT_ATTRIBUTES.ObjectName.deref
    try:
        object_name = object_name_struct.Buffer.deref.wstring.concrete
        logger.debug(f"ObjectName: {object_name}")
    except:
        return
    if 'tainted_unicode_strings' in state.globals:
        # Check if the ObjectName is controllable.
        if tmp_state.satisfiable() and (str(state.mem[ObjectName].struct._UNICODE_STRING.Buffer.resolved) in state.globals[
            'tainted_unicode_strings'] or tainted_buffer(state.memory.load(Buffer, 0x80))):
            ret_addr = hex(state.callstack.ret_addr)
            print_vuln(f'ObjectName in ObjectAttributes controllable', func_name, state, {
                'ObjectAttributes': {'ObjectName': str(ObjectName),
                                     'ObjectName.Buffer': str(state.memory.load(Buffer, 0x80).reversed),
                                     'Attributes': str(Attributes)}}, {'return address': ret_addr})
...
def print_vuln(title, description, state, parameters, others):
    ...
    if (title, state.addr, str(IoControlCode)) not in globals.vulns_unique:
        ...
        data['eval'] = {'IoControlCode': IoControlCode, 'SystemBuffer': SystemBuffer,
                        'Type3InputBuffer': Type3InputBuffer, 'UserBuffer': UserBuffer,
        ...
        if len(names) == 0:
            ...
            for i in names:
                if i:
                    ...
                    logger.debug(f'device name: {dd_name}')
...
def find_driver_type():
    ...
    if globals.proj.loader.find_symbol('IoCreateDevice'):
        logger.info(f'WDM driver: {globals.proj}')
        driver_type = 'wdm'
    else:
        logger.info(f'Different driver type detected: {globals.proj}')
