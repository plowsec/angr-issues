...
def wrmsr_hook(state):
    ...
    if utils.tainted_buffer(state.regs.eax) and utils.tainted_buffer(state.regs.ecx) and utils.tainted_buffer(
            state.regs.edx):
        # Check whether the regsiter is constrained.
        tmp_state = state.copy()
        tmp_state.solver.add(claripy.Or(tmp_state.regs.ecx == 0x00000174, tmp_state.regs.ecx == 0x00000175,
                                        tmp_state.regs.ecx == 0x00000176, tmp_state.regs.ecx == 0xC0000081,
                                        tmp_state.regs.ecx == 0xC0000082, tmp_state.regs.ecx == 0xC0000083))

        if tmp_state.satisfiable():
            utils.print_vuln('arbitrary wrmsr', '', state,
                             {'Register': str(state.regs.ecx), 'Value': (str(state.regs.edx), str(state.regs.eax))}, {})
...
def int_hook(state: angr.SimState):
    logger.warning(f'interrupt: {state.regs.rax}')
    return
